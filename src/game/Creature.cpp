...
bool Creature::IsImmuneToSpell(SpellEntry const* spellInfo)
{
    ...
    if ((!IsPet() || GetEntry() == 26126 || GetEntry() == 27829) && GetCreatureInfo()->MechanicImmuneMask & (1 << (spellInfo->Mechanic - 1)))
        return true;
...
{
    if ((!IsPet() || GetEntry() == 26126 || GetEntry() == 27829) && GetCreatureInfo()->MechanicImmuneMask & (1 << (spellInfo->EffectMechanic[index] - 1)))
        return true;
