...
uint32 Unit::DealDamage(Unit *pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const *spellProto, bool durabilityLoss)
{
    ...
    else                                                    // if (health <= damage)
    {
        ...
        {
            // interrupting auras with AURA_INTERRUPT_FLAG_DAMAGE before checking !damage (absorbed damage breaks that type of aura
            if (spellProto)
    ...
    if(pCaster != this)
    {
        ...
        for(AuraList::const_iterator i = vSplitDamageFlat.begin(), next; i != vSplitDamageFlat.end() && RemainingDamage >= 0; i = next)
        {
            ...
            if(!caster->IsImmunedToDamage(schoolMask))
            {
                ...
                pCaster->SendSpellMiss(caster, (*i)->GetId(), SPELL_MISS_IMMUNE);
        }
        ...
        for(AuraList::const_iterator i = vSplitDamagePct.begin(), next; i != vSplitDamagePct.end() && RemainingDamage >= 0; i = next)
        {
            ...
            else pCaster->SendSpellMiss(caster, (*i)->GetId(), SPELL_MISS_IMMUNE);
        }
    }
...
{
    if (IsNonMeleeSpellCasted(false) || hasUnitState(UNIT_STAT_NO_DEFENSE))
        return 0.0f;
...
Unit* Unit::SelectMagnetTarget(Unit *victim, Spell* spell, SpellEffectIndex eff)
{
    ...
    else
    {
        ...
        for(AuraList::const_iterator i = hitTriggerAuras.begin(); i != hitTriggerAuras.end(); ++i)
        {
            ...
            {
                if (magnet->isAlive() && (!spell || spell->CheckTarget(magnet, eff)))
                {
...
uint32 Unit::SpellDamageBonusDone(Unit *pVictim, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (GetTypeId()==TYPEID_UNIT && GetObjectGuid().IsVehicle())
        return pdamage;
...
void Unit::KnockBackFrom(Unit* target, float horizontalSpeed, float verticalSpeed)
{
    ...
    {
        float moveTimeHalf = 2 * verticalSpeed / Movement::gravity;
        float max_height = -Movement::computeFallElevation(moveTimeHalf,false,-verticalSpeed);

        float dis = moveTimeHalf * horizontalSpeed;
        float ox, oy, oz;
...
bool Unit::IsVisibleTargetForAoEDamage(WorldObject const* caster, SpellEntry const* spellInfo) const
{
    ...
    // spell can hit stealth/invisible targets, just check for LoS
        ...
        return spellInfo->AttributesEx2 & SPELL_ATTR_EX2_CANT_REFLECTED ? true : caster->IsWithinLOSInMap(this);
}
