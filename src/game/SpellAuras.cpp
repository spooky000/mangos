        ...
        switch (spellProto->SpellFamilyName)
        {
            ...
            case SPELLFAMILY_DRUID:
            {
                ...
                if (spellProto->SpellFamilyFlags.test<CF_DRUID_RIP_BITE>())
                {
                    ...
                    for(Unit::AuraList::const_iterator itr = dummyAuras.begin(); itr != dummyAuras.end(); ++itr)
                    {
                        if((*itr)->GetId()==34241 || (*itr)->GetId() == 60774)
                        {
...
void Aura::PeriodicTick()
{
    ...
    switch(m_modifier.m_auraname)
    {
        ...
        case SPELL_AURA_PERIODIC_DAMAGE_PERCENT:
        {
            ...
            target->CalculateDamageAbsorbAndResist(pCaster, GetSpellSchoolMask(spellProto), DOT, pdamage, &absorb, &resist, !(GetSpellProto()->AttributesEx2 & SPELL_ATTR_EX2_IGNORE_LOS));
        ...
        case SPELL_AURA_PERIODIC_HEALTH_FUNNEL:
        {
            ...
            target->CalculateDamageAbsorbAndResist(pCaster, GetSpellSchoolMask(spellProto), DOT, pdamage, &absorb, &resist, !(spellProto->AttributesEx2 & SPELL_ATTR_EX2_IGNORE_LOS));
            cleanDamage.absorb += absorb;
...
//                return;
            ...
            if (spell->SpellFamilyFlags.test<CF_DEATHKNIGHT_HYSTERIA>())
            {
                ...
                target->DealDamage(target, deal, NULL, NODAMAGE, SPELL_SCHOOL_MASK_NORMAL, NULL, false);
                return;
            }
            break;
        }
...
void SpellAuraHolder::HandleSpellSpecificBoosts(bool apply)
{
    ...
    switch(GetSpellProto()->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            if(GetSpellProto()->IsFitToFamilyMask(0x0, 0x2))
            {
                // Can't proc on self
                if (GetCasterGuid() == m_target->GetObjectGuid())
                    return;
...
void Aura::HandleDamagePercentTaken(bool apply, bool Real)
{
    ...
    if (apply)
    {
        ...
        // Icebound Fortitude
        else if (GetSpellProto()->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT && GetSpellProto()->SpellFamilyFlags & 0x00100000)
        {
            Unit * caster = GetCaster();
            if (!caster)
                return;
