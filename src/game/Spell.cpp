...
Spell::Spell( Unit* caster, SpellEntry const *info, bool triggered, ObjectGuid originalCasterGUID, SpellEntry const* triggeredBy )
{
    ...
    if (m_spellInfo->DmgClass == SPELL_DAMAGE_CLASS_MAGIC &&
        ...
        (m_spellInfo->SpellFamilyName == SPELLFAMILY_MAGE && m_spellInfo->SpellFamilyFlags.test<CF_MAGE_ARCANE_MISSILES2>())) // Tick of Arcane Missile should be reflectable
    {
...
void Spell::SetTargetMap(SpellEffectIndex effIndex, uint32 targetMode, UnitList& targetUnitMap)
{
    ...
    switch(targetMode)
    {
        ...
        case TARGET_EFFECT_SELECT:
        {
            ...
            if (m_spellInfo->IsFitToFamilyMask(0x0, 0x00000020) && m_spellInfo->SpellIconID == 3217)
            {
                targetUnitMap.push_back(m_caster);
                break;
            }
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        switch(m_spellInfo->Effect[i])
        {
            ...
            case SPELL_EFFECT_SKINNING:
            {
                ...
                if (creature->GetCreatureType() != CREATURE_TYPE_CRITTER && !creature->loot.isLooted() && !creature->lootForSkin)
    ...
    if (!(m_spellInfo->AttributesEx2 & SPELL_ATTR_EX2_CANT_REFLECTED) && !m_targets.getUnitTarget() && !m_targets.getGOTarget() && !m_targets.getItemTarget())
    {
...
bool Spell::CheckTarget( Unit* target, SpellEffectIndex eff )
{
    ...
    if ((m_spellInfo->Id == 2825 || m_spellInfo->Id == 32182)
